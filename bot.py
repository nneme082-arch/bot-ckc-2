...
"""
    ...
    def format_schedule_filtered(
        ...
        for day in days:
            ...
            visible_lessons = [l for l in day.get("lessons", []) if l.get("entries")]
            ...
            for lesson in visible_lessons:
                ...
                for i, entry in enumerate(lesson.get("entries", [])):
                    ...
                    lines.append(f"{prefix} {subj}{note_part}{teacher_part}{room_part}")

            lines.append("")
        return "\n".join(lines).strip()

    # ------------------------------------------------------------------
    # Запуск и основная петля
    # ------------------------------------------------------------------
    def wait_until_ready(self) -> Dict[str, Any]:
        while True:
            try:
                me = self.get_me()
                # Выполняем установку команд бота каждый раз (не страшно)
                self.api_call(
                    "setMyCommands",
                    {"commands": [
                        {"command": "schedule", "description": "Показать расписание"},
                        {"command": "setgroup", "description": "Выбрать группу"},
                        {"command": "mygroup", "description": "Текущая группа"},
                        {"command": "groups", "description": "Все группы"},
                        {"command": "find", "description": "Поиск группы"},
                        {"command": "refresh", "description": "Обновить кеш"},
                        {"command": "status", "description": "Состояние кеша"},
                        {"command": "stats", "description": "Статистика по предметам"},
                    ]},
                )
                return me
    ...
    def run(self, timeout: int) -> None:
        ...
        username = (self.me or {}).get("username", "unknown")
